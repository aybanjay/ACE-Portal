...
$sql = $conn->query("SELECT cv.*, alumnus_bio.* FROM cv INNER JOIN alumnus_bio ON alumnus_bio.id = cv.user_id WHERE alumnus_bio.id = '$id' AND cv.status = 1");

$row=$sql->fetch_assoc();

$fullname = $row['firstname']." ".$row['middlename']." ".$row['lastname'];
require_once('../cv/vendor/TCPDF/tcpdf.php'); 
